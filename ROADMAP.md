...
- **Ottimizzazioni PageSpeed & Performance Mobile/Desktop (22 maggio 2026)**:
  ...
  - **Bundle Code splitting (Rollup)**: Valutato il partizionamento dei chunk (`manualChunks`) in Rollup per l'isolamento delle librerie. Questa configurazione è stata disattivata raggruppando i file secondo gli algoritmi nativi standard di Vite per eliminare l'errore a runtime sullo scope d'inizializzazione temporale ("Cannot access before initialization" sui moduli vendor circular dependency). I benefici principali sui punteggi mobile (precaricamento font, rinvio script SendFox e corretto lazy loading nativo delle immagini) rimangono tutti pienamente attivi.
