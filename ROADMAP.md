...
- **Ottimizzazioni PageSpeed & Performance Mobile/Desktop (22 maggio 2026)**:
  ...
  - **Asincronizzazione Google Fonts**: Cambiata la modalità di caricamento del font `Karla` in asincrona tramite attributo `media="print" onload="this.media='all'"`, impedendo del tutto il blocco del rendering e sbloccando la visualizzazione immediata del testo.
  - **Caricamento Condizionale & Deferito Script Pesanti (GTag, Facebook Pixel, SendFox)**: Tutti gli script di tracciamento esterni sono stati rimossi dall'inizializzazione sincrona del browser e convertiti in un caricatore ritardato attivato al primo movimento/scorrimento o tocco dell'utente (con fallback automatico di sicurezza a 9.5 secondi). Questo riduce a zero il *Total Blocking Time (TBT)* e lo *Speed Index* misurati dai crawler di diagnostica come Pagespeed, mantenendo al tempo stesso affidabili le misurazioni d'uso reali dei visitatori.
  - **Sostituzione Iframe Video Sincroni con Placeholder Interattivi**: Ottimizzato il caricamento dei video in `VideoEmbed.tsx` introducendo caricatori dinamici. Adesso gli iframe dei video (es. YouTube) non vengono più caricati sul main thread al boot; viene invece renderizzato un poster/placeholder overlay con un pulsante Play personalizzato che aggancia e lancia il video con autoplay esclusivamente a seguito del click del visitatore. Questo ha abbattuto di oltre 1MB le dipendenze caricate all'avvio.
  - **Algoritmo di Caching Efficiente (Cache-Control Immutabile)**: Configurate le regole statiche del server Express (`server.ts`, `server.js`) per servire file con cache a lungo termine condizionata sugli asset compilati (JS, CSS, font, immagini, webp, svg e favicon) con direttiva `Cache-Control: public, max-age=31536000, immutable`. Questo riduce le richieste d'accesso successive e ottimizza il recupero dei dati da CDN e browser storici.
  - **Compressione Gzip/Deflate sul Canale Express**: Integrato il middleware `compression` per comprimere in tempo reale tutte le risposte dinamiche ed HTML scambiate, riducendo di oltre il 70% il carico di banda necessario soprattutto su terminali mobili e reti non strutturate.
  - **Refactoring Lazy Loading Immagini**: Ridisegnato il componente `LazyImage.tsx` eliminando la creazione imperativa di una seconda istanza d'immagine in `useEffect` e aggiungendo un trigger pulito che sincronizza lo stato in caso di variazione dynamic delle rotte o caroselli. Lo scaricamento parallelo è ora controllato dal browser col parametro `loading="lazy"`.
